package com.cse.warana.utility.AggregatedProfileGenerator.jate.core.voting;
...
/*		for (WeightedOutput output : outputs) {
			List<Term> PhraseExtractor.nlp_resources.test = outputs[1].getOutputList();
			PhraseExtractor.nlp_resources.test.removeAll(output.getOutputList());
			if (PhraseExtractor.nlp_resources.test.size() != 0)
...
/*
	...
	public static void main(String[] args) {
		try {
			...
			List<Term> tfidf = v.load("X:\\Work_stuff\\project_remote\\JTEC_SDK\\JTEC\\TEXTractor_v2.1\\PhraseExtractor.nlp_resources.test\\animal/TfIdf_ATR_ALGORITHM.txt");
			List<Term> weridness =v.load("X:\\Work_stuff\\project_remote\\JTEC_SDK\\JTEC\\TEXTractor_v2.1\\PhraseExtractor.nlp_resources.test\\animal/Weirdness_ATR_ALGORITHM.txt");
			List<Term> glossex = v.load("X:\\Work_stuff\\project_remote\\JTEC_SDK\\JTEC\\TEXTractor_v2.1\\PhraseExtractor.nlp_resources.test\\animal/IBM_GlossEx_ATR_ALGORITHM.txt");
			List<Term> termex = v.load("X:\\Work_stuff\\project_remote\\JTEC_SDK\\JTEC\\TEXTractor_v2.1\\PhraseExtractor.nlp_resources.test\\animal/TermEx_ATR_ALGORITHM.txt");
			List<Term> cvalue = v.load("X:\\Work_stuff\\project_remote\\JTEC_SDK\\JTEC\\TEXTractor_v2.1\\PhraseExtractor.nlp_resources.test\\animal/CValue_ALGORITHM.txt");
