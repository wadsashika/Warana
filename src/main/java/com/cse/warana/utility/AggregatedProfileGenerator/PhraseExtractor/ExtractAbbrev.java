...
public class ExtractAbbrev {
    ...
    public HashMap<String, String> extractAbbrPairs(String inFile,HashMap<String,String> pairs) {
                    ...
                    if (shortForm.length() > 0 || longForm.length() > 0) {
                        if (shortForm.length() > 1 && longForm.length() > 1) {
                            ...
                            if (isValidShortForm(shortForm)){
                                String line=extractAbbrPair(shortForm.trim(), longForm.trim());
                                if(line!=null)
    ...
    private String extractAbbrPair(String shortForm, String longForm) {
        ...
        if (testMode) {
            ...
            return shortForm.trim()+","+bestLongForm.trim();
        }
        return null;
    }
    ...
    public static void main(String[] args) {
        ...
        Vector candidates;
        String[] candidate;
        String filename =  "src\\main\\resources\\Docs\\SkillDocs\\.Net\\0.txt";
        ...
        extractAbbrev.extractAbbrPairs(filename,pairs);
        if (extractAbbrev.testMode)
    ...
    public void Extract(String srcPath, String destPath) {
        ...
        for (String fileName : file.list()) {
            extractAbbrPairs(srcPath+"/"+fileName,pairs);
        }
        fileManager.WriteAbbrFile(destPath,file.getName()+".csv", pairs);
    }
}
